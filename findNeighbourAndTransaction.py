...
def theLastMonth(sparkSession,hashId,liuShui,neighbours):
    ...
    print("几个邻居:",neighbours.count())
    neighbours=neighbours.select("id").withColumnRenamed("id","from").distinct()
    fromResult=liuShui.join(neighbours,on="from",how="inner")
    print("fromresult",fromResult.count())
    neighbours=neighbours.withColumnRenamed("from","to")
    toResult=liuShui.join(neighbours,on="to",how="inner")
    print("toresult",toResult.count())
...
def isInBlackList(sparkSession,neighbours,blackList,hashId):
    ...
    blackList = blackList.withColumn("blacklabel", F.lit(1))
    isInBlackListResult = neighbours.join(blackList, on="id", how="leftouter")
    isInBlackListResult = isInBlackListResult.withColumn("isInBlackListResult", col("blacklabel").isNotNull())
    isInBlackListResult=isInBlackListResult.drop("blacklabel")
